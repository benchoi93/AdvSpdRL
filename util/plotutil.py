...
def info_graph(env, veh_info, check_finish=False, path='./simulate_gif/info_graph.png'):
    # t1 = time.time()
    pos = veh_info[:, 0]
    vel = veh_info[:, 1]*3.6
    acc = veh_info[:, 2]
    ...
    # pos-vel
    section_max_speed = env.section.sms_list[int(step[-1]*10)][1]
    ...
    if check_finish == False:
        ...
        for i in range(cur_idx, min(cur_idx+env.num_action_unit+1, len(env.section.section_max_speed)-1)):
    ...
    ax3.plot([x*env.dt for x in range(len(pos))], pos, lw=2, color='k')
    green = env.signal[0].phase_length[True]
    red = env.signal[0].phase_length[False]
    cycle = green+red
    for j in range(env.num_signal):
        for i in range(int(env.timelimit/10/cycle)+1):
            ax3.plot(np.linspace(cycle*i-(cycle-env.signal[j].offset), cycle*i-(cycle-env.signal[j].offset)+green, green*10),
                     [env.signal[j].location]*(green*10), lw=2, color='g')
            ax3.plot(np.linspace(cycle*i-(cycle-env.signal[j].offset)+green, cycle*i-(cycle-env.signal[j].offset)+cycle, red*10),
    ...
    reward_min = np.round(np.min(env.reward_at_time[:, 1]), 0)-2
