...
for i in range(100):
    ...
    info_graph(env_list, [env.vehicle.veh_info[:env.timestep+1] for env in env_list],
               check_finish=True, path=f'{outpath}/{modelname}{cuda}/infograph_base_{i}.png')
    ...
    info_graph_detail(env_list, [env.vehicle.veh_info[:env.timestep+1] for env in env_list], True,
                      path=f'{outpath}/{modelname}{cuda}/infograph_detail_separate_{i}.png')
    info_graph_detail(env_list, [env.vehicle.veh_info[:env.timestep+1] for env in env_list], False,
                      path=f'{outpath}/{modelname}{cuda}/infograph_detail_nonseparate_{i}.png')
    # render_gif = True
    # if render_gif == True:
    #     env_num = 0
    #     for env in env_list:
